                ...
                if (i < input.size())
                {
                    ...
                    switch (command)
                    {
                        case 'm':
                        {
							...
							//else
								//result.append("\x1B[" + parameters + "m");
...
void TerminalWindow::SetCurrentDirectoryPath(const std::filesystem::path &path)
{
    ...
    m_pseudoTerminal->ReadOutput([this](const char *bytes, size_t size) {
        static bool recordingMSBuildOutput = false;
        ...
        if (!recordingMSBuildOutput && m_busy)
...
void TerminalWindow::RenderWindow(void)
{
    ...
    static auto inputTextCallback = [](ImGuiInputTextCallbackData* data) -> int32_t
    {
        ...
        {
            data->BufDirty = true;
            data->BufTextLen = commands[commands.size() - 1 - index].size();
            data->CursorPos = data->BufTextLen;
            strcpy(data->Buf, commands[commands.size() - 1 - index].c_str());
        };
        ...
        auto clearBuffer = [&](void) -> void
        {
            ...
            data->BufTextLen = 0;
            data->CursorPos = 0;
            memset(data->Buf, 0, strlen(data->Buf));
        };
    ...
    if (ImGui::Begin(m_windowTitle, &m_opened, ImGuiWindowFlags_NoNav))
    {
        ...
        if (ImGui::BeginChild("##ScrollingRegion", ImVec2(0, -ImGui::GetFrameHeightWithSpacing()), 0, ImGuiWindowFlags_AlwaysVerticalScrollbar | ImGuiWindowFlags_NoNav))
        {
			...
			bool inputApplied = isAnyKeyDown();
            if (!m_busy)
            {
                ...
                if (ImGui::BeginChild("##InputChild", ImVec2(0, ImGui::GetTextLineHeightWithSpacing() * 1.5), 0, ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
                {
                    ImGui::PushItemWidth(-1); 

                    if ((isFocused && inputApplied) || (isHovered && ImGui::IsAnyMouseDown()))
            ...
            if (ImGui::GetScrollY() >= ImGui::GetScrollMaxY() || inputApplied)
